...
import com.azamovhudstc.logosmart.data.local.shp.AppReference
...
fun String.screenCurrentEnum(): CurrentScreenEnum {
    return when (this) {
        ...
        else -> CurrentScreenEnum.REGISTER
    }
}
...
fun String.getYearCurrentLanguage():String{
    return if (AppReference(LogoApp.currentContext()!!).currentLanguage==LanguageType.ru){
        "$this-год"
    } else if (AppReference(LogoApp.currentContext()!!).currentLanguage==LanguageType.uz){
        "$this-yil"
